...
pub const mnemonics_map = struct {
    const table_info = table: {
        ...
        while (size_multiplier <= 35) : (size_multiplier += 1) {
            const t = generateSizedTableWithSeed(Token.mnemonics().len * size_multiplier);
            if (t[2]) break :table .{ .data = t[0], .seed = t[1] };
        }
        @compileError("mnemonics_map's size multiplier > 35");
    };

    fn generateSizedTableWithSeed(comptime size: usize) struct { [size]struct { u64, Token }, u32, bool } {
        const mnemonics = Token.mnemonics();
        var data = [_]struct { u64, Token }{.{ 0, .comma }} ** size;
        var fill: usize = 0;
        var seed: u32 = 0;
        while (fill != Token.mnemonics().len and seed < 15000) : (seed += 1) {
            fill = 0;
            ...
            for (mnemonics) |mnemonic| {
                ...
                if (data[i][0] == 0) {
                    data[i] = .{ full, @unionInit(Token, mnemonic.name, {}) };
                    fill += 1;
                } else break;
            }
        }
        return .{ data, seed - 1, fill == Token.mnemonics().len };
    }
